...
describe("Post Tests", () => {
    ...
    it("[11] Id'ye göre post listeleniyor mu?", async () => {
        const sample = { "Username": "Ali", "Password": "1234" }
        const loginRes = await superTest(server).post("/api/auth/login").send(sample);
...
describe("Comments Tests", () => {
    ...
    test("[16] Token gönderildiğinde comment ekleniyor mu?", async () => {
        const sampleLogin = { "Username": "Ali", "Password": "1234" }
...
describe("Users Tests", () => {
    ...
    test("[19] User Id'ye göre listeleniyor mu ?", async () => {
        ...
        expect(userList.body.UserEmail).toEqual("ali@ali")
    });

    test("[20] User güncelleniyor mu ?", async () => {
        const changes = { "Username": "AliAli", "UserEmail": "ali@ali", "Password": "1234" }
